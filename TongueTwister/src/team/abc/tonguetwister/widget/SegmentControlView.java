...
public class SegmentControlView extends View {
    ...
    public SegmentControlView(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SegmentControlView);

        String textArray = ta.getString(R.styleable.SegmentControlView_texts);
        if (textArray != null) {
            mTexts = textArray.split("\\|");
        }

        mTextSize = ta.getDimensionPixelSize(R.styleable.SegmentControlView_android_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context.getResources().getDisplayMetrics()));
        mCornerRadius = ta.getDimensionPixelSize(R.styleable.SegmentControlView_cornerRadius, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context.getResources().getDisplayMetrics()));
        mDirection = Direction.values()[ta.getInt(R.styleable.SegmentControlView_direction, 0)];

        mHorizonGap = ta.getDimensionPixelSize(R.styleable.SegmentControlView_horizonGap, 0);
        mVerticalGap = ta.getDimensionPixelSize(R.styleable.SegmentControlView_verticalGap, 0);

        int gap = ta.getDimensionPixelSize(R.styleable.SegmentControlView_gaps, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context.getResources().getDisplayMetrics()));
        ...
        mColors = ta.getColorStateList(R.styleable.SegmentControlView_colors);
        mSelectedTextColor = ta.getColor(R.styleable.SegmentControlView_textSelectedColors, Color.WHITE);
        if (mColors == null) {
            mColors = new ColorStateList(new int[][]{{}}, new int[]{0xFF0099CC});
        }

        mBoundWidth = ta.getDimensionPixelSize(R.styleable.SegmentControlView_boundWidth, mBoundWidth);
        mSeparatorWidth = ta.getDimensionPixelSize(R.styleable.SegmentControlView_separatorWidth, mSeparatorWidth);
